...
async def ايديي(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await register_user(update)
...
async def رسائلي(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await register_user(update)
    msgs = get_messages(update.effective_user.id)
    await reply(update, f"💬 رسائلك: {msgs}")

async def تفاعلي(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update, "🔥 تفاعلك: متوسط")  # ممكن تحسينه لاحقاً بناءً على الرسائل والنقاط

async def المجموعة(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    await reply(update, f"📛 اسم المجموعة: {chat.title}\n🆔 الايدي: {chat.id}")

async def الرابط(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update, "🔗 رابط المجموعة غير متوفر حاليا")

async def التاريخ(update: Update, context: ContextTypes.DEFAULT_TYPE):
    now = datetime.now().strftime("%Y-%m-%d ⏰ %H:%M")
    await reply(update, now)

# ===================== الردود المخصصة =====================
async def اضف_رد(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def حذف_رد(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
...
async def الردود_المتعددة(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
# ===================== منع وتنظيف =====================
async def منع(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def منع_بالرد(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.reply_to_message:
        uid = update.message.reply_to_message.from_user.id
        ban_user(uid)
        await reply(update,f"⛔ تم منع {uid}")

async def قائمة_المنع(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def مسح_قائمة_المنع(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cursor.execute('DELETE FROM banned')
    conn.commit()
    await reply(update,"🗑️ تم مسح قائمة المنع")

async def تنظيف(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
# ===================== ترفيه وألعاب =====================
async def غنيلي(update: Update, context: ContextTypes.DEFAULT_TYPE):
    songs = ["🎵 أغنية 1","🎵 أغنية 2","🎵 أغنية 3"]
    await reply(update, random.choice(songs))

async def فلم(update: Update, context: ContextTypes.DEFAULT_TYPE):
    movies = ["🎬 The Matrix","🎬 Inception","🎬 Interstellar"]
    await reply(update, random.choice(movies))

async def متحركة(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update, "🌀 صورة متحركة")

async def فيديو(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update, "📹 فيديو قادم قريباً")

async def رمزية(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply(update, "🖼️ صورة رمزية")

# ===================== القاموس الرئيسي بدون فواصل =====================
commands = {
    "اسمي": اسمي
    "ايديي": ايديي
    "نقاطي": نقاطي
